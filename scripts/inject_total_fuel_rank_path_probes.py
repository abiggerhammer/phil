...
(* Rank-path diagnostics.  The preceding higher-order application probes show
   ...
   facts, and locally derived fit arithmetic all seal quickly.  These probes
   isolate the remaining parent/child rank conversion boundary. *)

Lemma phase1_surface_nonterminal_goal_rank_decreases_probe :
  forall path name body child_rank parent_rank static_fuel,
...
Proof.
  intros path name body child_rank parent_rank static_fuel
    Hlookup Hchild_rank Hrank.
...
Lemma phase1_surface_budget_hypothesis_apply_decrease_opaque :
...
Proof.
  ...
  assert (Hchild_safe :
    ...
    abstract (
      ...
      exact Hlookup).
  pose proof
    (phase1_surface_nonterminal_goal_rank_decreases_probe
      path name body child_rank parent_rank static_fuel
      Hlookup Hchild_rank Hrank) as Hdecrease.
  assert (Hchild_fit :
