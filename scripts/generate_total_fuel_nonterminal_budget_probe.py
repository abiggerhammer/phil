...
Lemma phase1_surface_nonterminal_child_parse_from_budget :
  forall path name body input rest tree,
    ...
    (forall static_fuel rank budget,
      ...
      oracle_parse_fuel
        ...
        (GoalExpression (descend path (AtNonterminal name)) body)
        input = Some (rest, ResultTree tree)) ->
    forall static_fuel rank remaining,
      phase1_surface_parser_goal_rank_fuel
        static_fuel (GoalExpression path (ENonterminal name)) =
        Some rank ->
      phase1_surface_parser_local_measure input rank <= S remaining ->
      oracle_parse_fuel
        remaining
        phase1_surface_predictive_oracle
        phase1_surface_rules
...
Proof.
  intros path name body input rest tree Hlookup IHbody
    static_fuel rank remaining Hrank Hbudget.
...
Proof.
  ...
  - unfold phase1_surface_parser_local_measure in Hbudget.
    lia.
  - pose proof
      (phase1_surface_nonterminal_child_parse_from_budget
        path name body input rest tree Hlookup IHbody
        static_fuel rank remaining Hrank Hbudget) as Hchild_parse.
