...
The Phase 0 semantic session types the peer-visible payload as the nominal opaque type `DigestFailure`. The exact Phase 0 server does not inspect that value: its only use is forwarding it into the final `rejected(reason)` branch, and the exact Phase 0 client binds that payload and closes failure without inspecting it. Thus the current program exposes exactly one protocol-observable equivalence class at this boundary: **DigestMismatch**. This is an observational statement about the frozen Phase 0 program, not a claim that the abstract `DigestFailure` type has only one possible inhabitant in every future Phil program.

Rich validator diagnostics, implementation error objects, hash-library errors, and debug strings are not protocol data.
...
The first operand is the exact component transport handle. The second is the explicit Phase 0 wire reason code for the observable `DigestMismatch` class.
...
`digest-validation-v1` returns only the success predicate needed by the source control flow. The exact source does not perform any operation that can distinguish two failed `DigestMatches` results before forwarding the failure payload, and the peer does not inspect the payload after receiving it.

Rather than extend the digest ABI with a runtime-owned error object solely to preserve distinctions that the frozen Phase 0 program never observes, this profile chooses a canonical wire representative for that observable equivalence class:

```text
final DigestMatches failure, as observed by Phase 0
    -> DigestMismatch
    -> reason code 0x01
```

The code is derived from the exact digest-failure control-flow edge. This is not recovery from ambient state and does not add a runtime check. Any future source program that inspects `DigestFailure`, exposes more than one peer-visible digest failure class, or otherwise makes those distinctions observable requires a correspondingly richer semantic/representation boundary and a new ABI profile.
...
- reason code `0x01`: canonical Phase 0 `DigestMismatch` representation;
...
- explicit mapping of the frozen Phase 0 observable digest-failure class to `i8 0x01`;
