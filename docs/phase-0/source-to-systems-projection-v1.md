...
`Phil.Compiler.Phase0UploadProjection.projectPhase0UploadSources`:
...
5. computes a domain-separated digest over the exact client/server source texts;
6. rebinds the canonical Phase 0 base Systems artifact to that source digest;
...
Whitespace/comment-only changes may preserve the semantic trace, but they change the source-pair digest. Thus semantic identity and content identity remain distinct and explicit.
...
- the exact source bytes are content-bound;
