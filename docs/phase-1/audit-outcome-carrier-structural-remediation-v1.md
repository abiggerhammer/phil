...
`Phil.Surface.Check.Support` now derives the effective structural mode of a callable decision from the responsibilities retained in its `DecisionShape`.
...
The effective mode is applied both when deciding whether an expression result may be discarded and when the value is installed as a named binding. Existing variable movement then supplies the one-use behavior: a linear/affine carrier is consumed by its first read, an alias receives the same restricted mode, and final linear-completion checks reject an abandoned linear carrier.

This preserves the supported stored-once form (`let d = invoke ...; decide d ...`) instead of reducing the public checker to a direct-`decide invoke` syntax restriction. Ordinary Boolean decisions and responsibility-free callable decisions remain reusable.
...
The dedicated workflow also replays the existing CALL-019 branch resource, installation, proof-binding, and Surface decision-bridge controls.

## Assurance boundary

This repair establishes structural use-count discipline for the public supplied-contract Surface result carrier. It does not establish the semantic truth of supplied callable contracts, caller-resource relations, fact/obligation propositions, or callee lifecycle premises. It does not extend the enriched compiler bridge beyond its existing correlation contract, prove implementation/proof correspondence, or claim native/LLVM behavior. LLVM remains inside the Phase 1 trusted computing base.

The repair is intentionally local to carrier structural mode. It does not close other audit findings or refinement/proof-thread work.
