...
#define PHIL_ACCEPTED_RESPONSE_BYTES 17u
#define PHIL_ACCEPTED_TAG 0x01u
...
/* Private provider representation. Generated Phil code never sees this type. */
struct phil_upload_id_impl {
  uint8_t wire_token[PHIL_UPLOAD_ID_BYTES];
};
...
struct phil_smoke_state {
  ...
  unsigned invalid_accepted_transport_calls;
  unsigned invalid_accepted_id_calls;
...
void *phil_smoke_configure_accepted_response(
    const uint8_t *bytes,
    size_t available,
  ...
  for (index = 0; index < PHIL_UPLOAD_ID_BYTES; ++index) {
    smoke_state.upload_id.wire_token[index] = (uint8_t) (0xa0u + index);
  }
...
static bool common_store_observation(size_t expected_length) {
  return smoke_state.exact_receive_calls == 1
      ...
      && smoke_state.invalid_accepted_transport_calls == 0
      && smoke_state.invalid_accepted_id_calls == 0
...
bool phil_smoke_accepted_response_success_observed(
  ...
  return common_store_observation(payload_length)
      ...
      && (payload_length == 0
          || memcmp(smoke_state.stored_bytes, expected_payload, payload_length) == 0)
      && smoke_state.transport.output_length == response_length
      && response_length == PHIL_ACCEPTED_RESPONSE_BYTES
...
void phil_runtime_select_accepted(void *transport, void *upload_id_handle) {
  struct phil_transport_impl *destination = transport;
  struct phil_upload_id_impl *upload_id = upload_id_handle;

  if (destination != &smoke_state.transport) {
    smoke_state.invalid_accepted_transport_calls += 1;
    return;
  }
  if (upload_id_handle != smoke_state.last_returned_upload_id
      || upload_id != &smoke_state.upload_id
      || smoke_state.configured_store_status != 1) {
    smoke_state.invalid_accepted_id_calls += 1;
    return;
  }

  destination->output[0] = PHIL_ACCEPTED_TAG;
  memcpy(destination->output + 1, upload_id->wire_token, PHIL_UPLOAD_ID_BYTES);
  destination->output_length = PHIL_ACCEPTED_RESPONSE_BYTES;
  smoke_state.accepted_calls += 1;
}
