#ifndef PHIL_PHASE0_DIGEST_VALIDATION_V1_H
#define PHIL_PHASE0_DIGEST_VALIDATION_V1_H
...
/* Runtime ABI exercised by the digest-validation candidate. */
...
void *phil_smoke_configure_digest_transport(
  const uint8_t *bytes,
  size_t available,
  uint64_t requested_length,
  const uint8_t expected_digest[32]);
bool phil_smoke_digest_match_observed(size_t expected_length);
bool phil_smoke_digest_mismatch_observed(size_t expected_length);
