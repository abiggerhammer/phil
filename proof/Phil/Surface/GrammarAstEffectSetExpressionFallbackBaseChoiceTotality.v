...
(*
  Converse for the fallback/base-choice lift through effect sets from #1329.

  The carrier introduced there advances each expression-outer effect-set member
  to the fallback/base-choice effect-expression carrier while preserving member
  order, punctuation, and the unchanged static-reference branch exactly. This
  file proves that refinement is total for derivable effect_set_expression trees.
*)

Lemma
  phase1_surface_normalize_effect_expression_expression_fallback_base_choice_values_total_from_suffix_repetition :
  forall path body input rest trees,
    ...
    forall effects,
      trees =
        map phase1_surface_effect_expression_expression_outer_suffix_tree effects ->
      exists refined,
...
Proof.
  ...
  - destruct effects as [|effect effects]; cbn in Htrees;
    ...
    unfold phase1_surface_effect_expression_expression_outer_suffix_tree in Hitem.
    destruct
...
Lemma
  phase1_surface_normalize_effect_expression_expression_fallback_base_choice_values_total_from_entries_derivation :
...
Lemma
  phase1_surface_normalize_effect_set_expression_fallback_base_choice_literal_spine_total_from_spine_derivation :
...
Proof.
  intros [effects] path input rest Hderive.
  unfold phase1_surface_effect_set_expression_outer_literal_spine_tree in Hderive.
  cbn in Hderive.
  destruct
...
Proof.
  ...
  - cbn in Hderive |- *.
    destruct
      (derives_nonterminal_exposes_body
        ...
        (PTNonterminal "effect_set_expression"
          (PTAlternative 0
            (phase1_surface_effect_set_expression_outer_literal_spine_tree
              literal)))
        Hderive)
      as [body [subtree [Hlookup [Htree Hbody]]]].
    ...
    exists (Phase1EffectSetExpressionFallbackBaseChoiceLiteral refined_literal).
    rewrite Hrefined_literal.
    reflexivity.
  - exists (Phase1EffectSetExpressionFallbackBaseChoiceReference reference).
    reflexivity.
Qed.
...
Proof.
  ...
  assert (Hnormalize :
    phase1_surface_normalize_effect_set_expression_fallback_base_choice_tree tree =
      Some refined).
  {
    unfold phase1_surface_normalize_effect_set_expression_fallback_base_choice_tree.
    rewrite Hbase.
    exact Hrefined.
  }
  exists refined.
