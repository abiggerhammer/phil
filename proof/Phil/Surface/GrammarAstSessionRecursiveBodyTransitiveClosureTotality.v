From Stdlib Require Import Arith.PeanoNat Lists.List Strings.String Lia.

From Phil.Surface Require Import
  GrammarAstSessionRecursiveBodyTransitiveClosureSpine
  GrammarAstSessionRecursiveBodyMutualAdapterTotality
  GrammarAstSessionRecursiveBodyMutualRecursiveClosureTotality
  GrammarAstSessionRecursiveBodyChoiceOneStepCarrierTotality
  GrammarAstSessionRecursiveBodyTransferRecursiveCarrierTotality
  GrammarAstSessionRecursiveBodyOneStepCarrierTotality
  GrammarAstSessionRecursiveOneStepCarrierTotality
  GrammarAstSessionMutualRecursiveClosureTotality
  GrammarAstSessionTransferRecursiveClosureTotality
  GrammarAstSessionBranchTotality.

Import ListNotations.
Open Scope string_scope.

(*
  Converse and finite-fuel closure for the final transitive recursive-body
  carrier from #1142.

  The four adapter fuels are collapsed internally to one common upper bound.
  A second fuel controls structural traversal through transfer continuations,
  choice-branch continuations, and recursive payload bodies.  This keeps the
  converse two-dimensional while preserving the five explicit fuel parameters
  of the forward normalizer.
*)

Lemma phase1_surface_normalize_recursive_body_mutual_adapter_fuels_monotone :
  forall source_fuel larger_source closure_fuel larger_closure
    transfer_fuel larger_transfer mutual_fuel larger_mutual
    session refined,
    source_fuel <= larger_source ->
    closure_fuel <= larger_closure ->
    transfer_fuel <= larger_transfer ->
    mutual_fuel <= larger_mutual ->
    phase1_surface_normalize_recursive_body_mutual_adapter_fuels
      source_fuel closure_fuel transfer_fuel mutual_fuel session = Some refined ->
    phase1_surface_normalize_recursive_body_mutual_adapter_fuels
      larger_source larger_closure larger_transfer larger_mutual session =
      Some refined.
Proof.
  intros source_fuel larger_source closure_fuel larger_closure
    transfer_fuel larger_transfer mutual_fuel larger_mutual
    session refined Hsource Hclosure Htransfer Hmutual Hnormalize.
  unfold phase1_surface_normalize_recursive_body_mutual_adapter_fuels
    in Hnormalize |- *.
  destruct
    (phase1_surface_normalize_recursive_one_step_session_spine session)
    as [shell |] eqn:Hshell; try discriminate Hnormalize.
  rewrite Hshell.
  destruct
    (phase1_surface_normalize_recursive_body_one_step_session_spine_fuels
      source_fuel closure_fuel shell)
    as [body_step |] eqn:Hbody; try discriminate Hnormalize.
  pose proof
    (phase1_surface_normalize_recursive_body_one_step_session_spine_fuels_monotone
      source_fuel larger_source closure_fuel larger_closure
      shell body_step Hsource Hclosure Hbody)
    as Hbody_larger.
  rewrite Hbody_larger.
  destruct
    (phase1_surface_normalize_recursive_body_transfer_recursive_session_spine_fuels
      source_fuel closure_fuel transfer_fuel body_step)
    as [transfer_step |] eqn:Htransfer_step; try discriminate Hnormalize.
  pose proof
    (phase1_surface_normalize_recursive_body_transfer_recursive_session_spine_fuels_body_monotone
      source_fuel larger_source closure_fuel larger_closure
      transfer_fuel body_step transfer_step
      Hsource Hclosure Htransfer_step)
    as Htransfer_body_larger.
  pose proof
    (phase1_surface_normalize_recursive_body_transfer_recursive_session_spine_fuels_transfer_monotone
      larger_source larger_closure transfer_fuel larger_transfer
      body_step transfer_step Htransfer Htransfer_body_larger)
    as Htransfer_larger.
  rewrite Htransfer_larger.
  destruct
    (phase1_surface_normalize_recursive_body_choice_one_step_session_spine_fuels
      source_fuel closure_fuel transfer_fuel transfer_step)
    as [choice_step |] eqn:Hchoice; try discriminate Hnormalize.
  pose proof
    (phase1_surface_normalize_recursive_body_choice_one_step_session_spine_fuels_monotone
      source_fuel larger_source closure_fuel larger_closure
      transfer_fuel larger_transfer transfer_step choice_step
      Hsource Hclosure Htransfer Hchoice)
    as Hchoice_larger.
  rewrite Hchoice_larger.
  eapply
    phase1_surface_normalize_recursive_body_mutual_recursive_session_spine_fuels_monotone.
  - exact Hsource.
  - exact Hclosure.
  - exact Htransfer.
  - exact Hmutual.
  - exact Hnormalize.
Qed.

Definition phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel
  (fuel : nat)
  (session : Phase1SurfaceContinueMutualRecursiveSessionSpine)
  : option Phase1SurfaceRecursiveBodyMutualRecursiveSessionSpine :=
  phase1_surface_normalize_recursive_body_mutual_adapter_fuels
    fuel fuel fuel fuel session.

Lemma phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel_monotone :
  forall fuel larger session refined,
    fuel <= larger ->
    phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel
      fuel session = Some refined ->
    phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel
      larger session = Some refined.
Proof.
  intros fuel larger session refined Hle Hnormalize.
  unfold phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel
    in Hnormalize |- *.
  eapply phase1_surface_normalize_recursive_body_mutual_adapter_fuels_monotone.
  - exact Hle.
  - exact Hle.
  - exact Hle.
  - exact Hle.
  - exact Hnormalize.
Qed.

Lemma phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel_total_from_derivation :
  forall path input rest session,
    Derives phase1_surface_rules path
      (ENonterminal "session_expression") input rest
      (phase1_surface_continue_mutual_recursive_session_spine_tree session) ->
    exists fuel refined,
      phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel
        fuel session = Some refined /\
      phase1_surface_recursive_body_mutual_recursive_session_spine_tree refined =
        phase1_surface_continue_mutual_recursive_session_spine_tree session.
Proof.
  intros path input rest session Hderive.
  destruct
    (phase1_surface_normalize_recursive_body_mutual_adapter_fuels_total_from_derivation
      path input rest session Hderive)
    as [source_fuel [closure_fuel [transfer_fuel
      [mutual_fuel [refined [Hnormalize Htree]]]]]].
  let fuel := constr:(Nat.max
    (Nat.max source_fuel closure_fuel)
    (Nat.max transfer_fuel mutual_fuel)) in
  assert (Hsource : source_fuel <= fuel).
  {
    eapply Nat.le_trans.
    - apply Nat.le_max_l.
    - apply Nat.le_max_l.
  }
  assert (Hclosure : closure_fuel <= fuel).
  {
    eapply Nat.le_trans.
    - apply Nat.le_max_r.
    - apply Nat.le_max_l.
  }
  assert (Htransfer : transfer_fuel <= fuel).
  {
    eapply Nat.le_trans.
    - apply Nat.le_max_l.
    - apply Nat.le_max_r.
  }
  assert (Hmutual : mutual_fuel <= fuel).
  {
    eapply Nat.le_trans.
    - apply Nat.le_max_r.
    - apply Nat.le_max_r.
  }
  exists fuel, refined.
  split.
  - unfold phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel.
    eapply phase1_surface_normalize_recursive_body_mutual_adapter_fuels_monotone.
    + exact Hsource.
    + exact Hclosure.
    + exact Htransfer.
    + exact Hmutual.
    + exact Hnormalize.
  - exact Htree.
Qed.

Definition phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
  (adapter_fuel traversal_fuel : nat)
  (session : Phase1SurfaceRecursiveBodyMutualRecursiveSessionSpine)
  : option Phase1SurfaceRecursiveBodyTransitiveSessionSpine :=
  phase1_surface_normalize_recursive_body_transitive_session_spine_fuel
    adapter_fuel adapter_fuel adapter_fuel adapter_fuel traversal_fuel session.

Lemma phase1_surface_normalize_recursive_body_transitive_session_branch_with_respects :
  forall normalize_left normalize_right branch refined,
    (forall continuation converted,
      normalize_left continuation = Some converted ->
      normalize_right continuation = Some converted) ->
    phase1_surface_normalize_recursive_body_transitive_session_branch_with
      normalize_left branch = Some refined ->
    phase1_surface_normalize_recursive_body_transitive_session_branch_with
      normalize_right branch = Some refined.
Proof.
  intros normalize_left normalize_right
    [label params boundary guard continuation] refined Hrespect Hnormalize.
  cbn in Hnormalize |- *.
  destruct (normalize_left continuation)
    as [converted |] eqn:Hconverted; try discriminate Hnormalize.
  inversion Hnormalize; subst refined.
  rewrite (Hrespect continuation converted Hconverted).
  reflexivity.
Qed.

Lemma phase1_surface_normalize_recursive_body_transitive_session_branch_tail_with_respects :
  forall normalize_left normalize_right branches refined,
    (forall continuation converted,
      normalize_left continuation = Some converted ->
      normalize_right continuation = Some converted) ->
    phase1_surface_normalize_recursive_body_transitive_session_branch_tail_with
      normalize_left branches = Some refined ->
    phase1_surface_normalize_recursive_body_transitive_session_branch_tail_with
      normalize_right branches = Some refined.
Proof.
  intros normalize_left normalize_right branches.
  induction branches as [|branch rest IH];
    intros refined Hrespect Hnormalize.
  - cbn in Hnormalize.
    inversion Hnormalize; subst refined.
    reflexivity.
  - cbn in Hnormalize |- *.
    destruct
      (phase1_surface_normalize_recursive_body_transitive_session_branch_with
        normalize_left branch)
      as [refined_branch |] eqn:Hbranch; try discriminate Hnormalize.
    destruct
      (phase1_surface_normalize_recursive_body_transitive_session_branch_tail_with
        normalize_left rest)
      as [refined_rest |] eqn:Hrest; try discriminate Hnormalize.
    inversion Hnormalize; subst refined.
    rewrite
      (phase1_surface_normalize_recursive_body_transitive_session_branch_with_respects
        normalize_left normalize_right branch refined_branch Hrespect Hbranch).
    rewrite (IH refined_rest Hrespect Hrest).
    reflexivity.
Qed.

Lemma phase1_surface_normalize_recursive_body_transitive_session_choice_with_respects :
  forall normalize_left normalize_right choice refined,
    (forall continuation converted,
      normalize_left continuation = Some converted ->
      normalize_right continuation = Some converted) ->
    phase1_surface_normalize_recursive_body_transitive_session_choice_with
      normalize_left choice = Some refined ->
    phase1_surface_normalize_recursive_body_transitive_session_choice_with
      normalize_right choice = Some refined.
Proof.
  intros normalize_left normalize_right
    [direction first_branch rest_branches] refined Hrespect Hnormalize.
  cbn in Hnormalize |- *.
  destruct
    (phase1_surface_normalize_recursive_body_transitive_session_branch_with
      normalize_left first_branch)
    as [refined_first |] eqn:Hfirst; try discriminate Hnormalize.
  destruct
    (phase1_surface_normalize_recursive_body_transitive_session_branch_tail_with
      normalize_left rest_branches)
    as [refined_rest |] eqn:Hrest; try discriminate Hnormalize.
  inversion Hnormalize; subst refined.
  rewrite
    (phase1_surface_normalize_recursive_body_transitive_session_branch_with_respects
      normalize_left normalize_right first_branch refined_first Hrespect Hfirst).
  rewrite
    (phase1_surface_normalize_recursive_body_transitive_session_branch_tail_with_respects
      normalize_left normalize_right rest_branches refined_rest Hrespect Hrest).
  reflexivity.
Qed.

Lemma phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels_monotone :
  forall adapter_fuel larger_adapter traversal_fuel larger_traversal
    session refined,
    adapter_fuel <= larger_adapter ->
    traversal_fuel <= larger_traversal ->
    phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
      adapter_fuel traversal_fuel session = Some refined ->
    phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
      larger_adapter larger_traversal session = Some refined.
Proof.
  intros adapter_fuel larger_adapter traversal_fuel.
  revert adapter_fuel larger_adapter.
  induction traversal_fuel as [|remaining IH];
    intros adapter_fuel larger_adapter larger_traversal
      session refined Hadapter Htraversal Hnormalize.
  - discriminate Hnormalize.
  - destruct larger_traversal as [|larger_remaining].
    + lia.
    + assert (Hremaining : remaining <= larger_remaining) by lia.
      unfold
        phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
        in Hnormalize |- *.
      cbn [phase1_surface_normalize_recursive_body_transitive_session_spine_fuel]
        in Hnormalize |- *.
      let normalize_left := constr:(
        phase1_surface_normalize_recursive_body_transitive_session_spine_fuel
          adapter_fuel adapter_fuel adapter_fuel adapter_fuel remaining) in
      let normalize_right := constr:(
        phase1_surface_normalize_recursive_body_transitive_session_spine_fuel
          larger_adapter larger_adapter larger_adapter larger_adapter
          larger_remaining) in
      assert (Hcontinuation :
        forall continuation converted,
          normalize_left continuation = Some converted ->
          normalize_right continuation = Some converted).
      {
        intros continuation converted Hconverted.
        change
          (phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
            adapter_fuel remaining continuation = Some converted)
          in Hconverted.
        change
          (phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
            larger_adapter larger_remaining continuation = Some converted).
        eapply IH.
        - exact Hadapter.
        - exact Hremaining.
        - exact Hconverted.
      }
      destruct session as [nonreference | reference_tree].
      * destruct nonreference as
          [direction parameter boundary guard continuation
          |choice
          |choice
          |terminal
          |payload
          |payload].
        -- destruct (normalize_left continuation)
             as [converted |] eqn:Hconverted; try discriminate Hnormalize.
           inversion Hnormalize; subst refined.
           rewrite (Hcontinuation continuation converted Hconverted).
           reflexivity.
        -- destruct
             (phase1_surface_normalize_recursive_body_transitive_session_choice_with
               normalize_left choice)
             as [converted |] eqn:Hchoice; try discriminate Hnormalize.
           inversion Hnormalize; subst refined.
           pose proof
             (phase1_surface_normalize_recursive_body_transitive_session_choice_with_respects
               normalize_left normalize_right choice converted
               Hcontinuation Hchoice) as Hchoice_larger.
           rewrite Hchoice_larger.
           reflexivity.
        -- destruct
             (phase1_surface_normalize_recursive_body_transitive_session_choice_with
               normalize_left choice)
             as [converted |] eqn:Hchoice; try discriminate Hnormalize.
           inversion Hnormalize; subst refined.
           pose proof
             (phase1_surface_normalize_recursive_body_transitive_session_choice_with_respects
               normalize_left normalize_right choice converted
               Hcontinuation Hchoice) as Hchoice_larger.
           rewrite Hchoice_larger.
           reflexivity.
        -- inversion Hnormalize; subst refined. reflexivity.
        -- destruct
             (phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel
               adapter_fuel (phase1_recursive_body_one_step_body payload))
             as [body_step |] eqn:Hbody_step; try discriminate Hnormalize.
           pose proof
             (phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel_monotone
               adapter_fuel larger_adapter
               (phase1_recursive_body_one_step_body payload)
               body_step Hadapter Hbody_step)
             as Hbody_step_larger.
           destruct (normalize_left body_step)
             as [converted_body |] eqn:Hbody; try discriminate Hnormalize.
           inversion Hnormalize; subst refined.
           rewrite Hbody_step_larger.
           rewrite (Hcontinuation body_step converted_body Hbody).
           reflexivity.
        -- inversion Hnormalize; subst refined. reflexivity.
      * inversion Hnormalize; subst refined.
        reflexivity.
Qed.

Definition phase1_surface_normalize_recursive_body_transitive_session_branch_fuels
  (adapter_fuel traversal_fuel : nat)
  (branch : Phase1SurfaceRecursiveBodyMutualRecursiveSessionBranchSpine)
  : option Phase1SurfaceRecursiveBodyTransitiveSessionBranchSpine :=
  phase1_surface_normalize_recursive_body_transitive_session_branch_with
    (phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
      adapter_fuel traversal_fuel)
    branch.

Definition phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels
  (adapter_fuel traversal_fuel : nat)
  (branches : Phase1SurfaceRecursiveBodyMutualRecursiveSessionBranchTailSpine)
  : option Phase1SurfaceRecursiveBodyTransitiveSessionBranchTailSpine :=
  phase1_surface_normalize_recursive_body_transitive_session_branch_tail_with
    (phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
      adapter_fuel traversal_fuel)
    branches.

Definition phase1_surface_normalize_recursive_body_transitive_session_choice_fuels
  (adapter_fuel traversal_fuel : nat)
  (choice : Phase1SurfaceRecursiveBodyMutualRecursiveSessionChoiceSpine)
  : option Phase1SurfaceRecursiveBodyTransitiveSessionChoiceSpine :=
  phase1_surface_normalize_recursive_body_transitive_session_choice_with
    (phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
      adapter_fuel traversal_fuel)
    choice.

Lemma phase1_surface_normalize_recursive_body_transitive_session_branch_fuels_monotone :
  forall adapter_fuel larger_adapter traversal_fuel larger_traversal
    branch refined,
    adapter_fuel <= larger_adapter ->
    traversal_fuel <= larger_traversal ->
    phase1_surface_normalize_recursive_body_transitive_session_branch_fuels
      adapter_fuel traversal_fuel branch = Some refined ->
    phase1_surface_normalize_recursive_body_transitive_session_branch_fuels
      larger_adapter larger_traversal branch = Some refined.
Proof.
  intros adapter_fuel larger_adapter traversal_fuel larger_traversal
    branch refined Hadapter Htraversal Hnormalize.
  unfold phase1_surface_normalize_recursive_body_transitive_session_branch_fuels
    in Hnormalize |- *.
  eapply phase1_surface_normalize_recursive_body_transitive_session_branch_with_respects.
  - intros continuation converted Hconverted.
    eapply
      phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels_monotone.
    + exact Hadapter.
    + exact Htraversal.
    + exact Hconverted.
  - exact Hnormalize.
Qed.

Lemma phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels_monotone :
  forall adapter_fuel larger_adapter traversal_fuel larger_traversal
    branches refined,
    adapter_fuel <= larger_adapter ->
    traversal_fuel <= larger_traversal ->
    phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels
      adapter_fuel traversal_fuel branches = Some refined ->
    phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels
      larger_adapter larger_traversal branches = Some refined.
Proof.
  intros adapter_fuel larger_adapter traversal_fuel larger_traversal
    branches refined Hadapter Htraversal Hnormalize.
  unfold
    phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels
    in Hnormalize |- *.
  eapply
    phase1_surface_normalize_recursive_body_transitive_session_branch_tail_with_respects.
  - intros continuation converted Hconverted.
    eapply
      phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels_monotone.
    + exact Hadapter.
    + exact Htraversal.
    + exact Hconverted.
  - exact Hnormalize.
Qed.

Lemma phase1_surface_normalize_recursive_body_transitive_session_choice_fuels_monotone :
  forall adapter_fuel larger_adapter traversal_fuel larger_traversal
    choice refined,
    adapter_fuel <= larger_adapter ->
    traversal_fuel <= larger_traversal ->
    phase1_surface_normalize_recursive_body_transitive_session_choice_fuels
      adapter_fuel traversal_fuel choice = Some refined ->
    phase1_surface_normalize_recursive_body_transitive_session_choice_fuels
      larger_adapter larger_traversal choice = Some refined.
Proof.
  intros adapter_fuel larger_adapter traversal_fuel larger_traversal
    choice refined Hadapter Htraversal Hnormalize.
  unfold phase1_surface_normalize_recursive_body_transitive_session_choice_fuels
    in Hnormalize |- *.
  eapply phase1_surface_normalize_recursive_body_transitive_session_choice_with_respects.
  - intros continuation converted Hconverted.
    eapply
      phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels_monotone.
    + exact Hadapter.
    + exact Htraversal.
    + exact Hconverted.
  - exact Hnormalize.
Qed.

Lemma phase1_surface_recursive_body_mutual_branch_exposes_continuation :
  forall path input rest branch,
    Derives phase1_surface_rules path
      (ENonterminal "session_branch") input rest
      (phase1_surface_recursive_body_mutual_recursive_session_branch_spine_tree branch) ->
    exists continuation_path continuation_input continuation_rest,
      Derives phase1_surface_rules continuation_path
        (ENonterminal "session_expression")
        continuation_input continuation_rest
        (phase1_surface_recursive_body_mutual_recursive_session_spine_tree
          (match branch with
           | Phase1RecursiveBodyMutualRecursiveBranch _ _ _ _ continuation =>
               continuation
           end)) /\
      List.length continuation_input <= List.length input.
Proof.
  intros path input rest
    [label params boundary guard continuation] Hderive.
  destruct
    (derives_nonterminal_exposes_body
      phase1_surface_rules path "session_branch"
      input rest
      (phase1_surface_recursive_body_mutual_recursive_session_branch_spine_tree
        (Phase1RecursiveBodyMutualRecursiveBranch
          label params boundary guard continuation))
      Hderive)
    as [body [subtree [Hlookup [Hnode Hbody]]]].
  rewrite phase1_surface_session_branch_lookup_for_totality in Hlookup.
  inversion Hlookup; subst body.
  unfold phase1_surface_session_branch_expression_for_totality in Hbody.
  cbn in Hnode.
  inversion Hnode; subst subtree.
  destruct
    (derives_sequence_expression_exposes_items
      phase1_surface_rules
      (descend path (AtNonterminal "session_branch"))
      [ ENonterminal "identifier";
        phase1_surface_session_branch_params_expression_for_totality;
        phase1_surface_session_branch_boundary_expression_for_totality;
        phase1_surface_session_branch_guard_expression_for_totality;
        ELiteral "=>";
        ENonterminal "session_expression"
      ]
      input rest
      (PTSequence
        [ phase1_surface_identifier_tree label;
          phase1_surface_session_branch_params_tree params;
          phase1_surface_boundary_refined_annotation_tree boundary;
          phase1_surface_guard_refined_annotation_tree guard;
          PTLiteral "=>";
          phase1_surface_recursive_body_mutual_recursive_session_spine_tree continuation
        ])
      Hbody)
    as [trees [Htree Hitems]].
  cbn in Htree.
  inversion Htree; subst trees.
  repeat match goal with
  | Hseq : DerivesSequence phase1_surface_rules _ _ (_ :: _) _ _ _ |- _ =>
      inversion Hseq; subst; clear Hseq
  end.
  match goal with
  | Hnil : DerivesSequence phase1_surface_rules _ _ [] _ _ _ |- _ =>
      inversion Hnil; subst; clear Hnil
  end.
  match goal with
  | Hcontinuation : Derives phase1_surface_rules ?continuation_path
      (ENonterminal "session_expression")
      ?continuation_input ?continuation_rest
      (phase1_surface_recursive_body_mutual_recursive_session_spine_tree
        continuation) |- _ =>
      exists continuation_path, continuation_input, continuation_rest;
      split;
      [ exact Hcontinuation
      | pose proof
          (phase1_surface_derivation_rest_length_le_input
            _ _ _ _ _ Hcontinuation) as Hlength;
        lia ]
  end.
Qed.

Lemma phase1_surface_recursive_body_one_step_payload_exposes_body_shorter :
  forall path input rest payload,
    Derives phase1_surface_rules path
      phase1_surface_recursive_session_expression_for_totality
      input rest
      (phase1_surface_recursive_body_one_step_payload_spine_tree payload) ->
    exists body_path body_input body_rest,
      Derives phase1_surface_rules body_path
        (ENonterminal "session_expression")
        body_input body_rest
        (phase1_surface_continue_mutual_recursive_session_spine_tree
          (phase1_recursive_body_one_step_body payload)) /\
      List.length body_input < List.length input.
Proof.
  intros path input rest [name body] Hderive.
  unfold phase1_surface_recursive_session_expression_for_totality in Hderive.
  unfold phase1_surface_recursive_body_one_step_payload_spine_tree in Hderive.
  destruct
    (derives_sequence_expression_exposes_items
      phase1_surface_rules path
      [ ELiteral "recursive";
        ENonterminal "identifier";
        ELiteral "=";
        ENonterminal "session_expression"
      ]
      input rest
      (PTSequence
        [ PTLiteral "recursive";
          phase1_surface_identifier_tree name;
          PTLiteral "=";
          phase1_surface_continue_mutual_recursive_session_spine_tree body
        ])
      Hderive)
    as [trees [Htree Hitems]].
  destruct
    (derives_sequence_cons_exposes_head_exact
      phase1_surface_rules path 0
      (ELiteral "recursive")
      [ ENonterminal "identifier";
        ELiteral "=";
        ENonterminal "session_expression" ]
      input rest trees Hitems)
    as [after_keyword [keyword_tree [tail1_trees
      [Htrees [Hkeyword Htail1]]]]].
  destruct
    (derives_sequence_cons_exposes_head_exact
      phase1_surface_rules path 1
      (ENonterminal "identifier")
      [ ELiteral "=";
        ENonterminal "session_expression" ]
      after_keyword rest tail1_trees Htail1)
    as [after_name [name_tree [tail2_trees
      [Htail1_trees [Hname Htail2]]]]].
  destruct
    (derives_sequence_cons_exposes_head_exact
      phase1_surface_rules path 2
      (ELiteral "=")
      [ ENonterminal "session_expression" ]
      after_name rest tail2_trees Htail2)
    as [after_equals [equals_tree [tail3_trees
      [Htail2_trees [Hequals Htail3]]]]].
  destruct
    (derives_sequence_cons_exposes_head_exact
      phase1_surface_rules path 3
      (ENonterminal "session_expression") []
      after_equals rest tail3_trees Htail3)
    as [after_body [actual_body_tree [nil_trees
      [Htail3_trees [Hbody Hnil]]]]].
  inversion Hnil; subst nil_trees.
  rewrite Htrees, Htail1_trees, Htail2_trees, Htail3_trees in Htree.
  inversion Htree; subst actual_body_tree.
  destruct
    (literal_derivation_is_exact
      phase1_surface_rules _ "recursive"
      input after_keyword keyword_tree Hkeyword)
    as [keyword_tail [Hinput [Hafter_keyword Hkeyword_tree]]].
  pose proof
    (phase1_surface_derivation_rest_length_le_input
      _ _ _ _ _ Hname) as Hname_length.
  pose proof
    (phase1_surface_derivation_rest_length_le_input
      _ _ _ _ _ Hequals) as Hequals_length.
  exists _, after_equals, after_body.
  split.
  - exact Hbody.
  - rewrite Hinput, Hafter_keyword.
    cbn.
    lia.
Qed.

Lemma phase1_surface_normalize_recursive_body_transitive_session_branch_fuels_total_from_derivation :
  forall parent_length path input rest branch,
    List.length input < parent_length ->
    Derives phase1_surface_rules path
      (ENonterminal "session_branch") input rest
      (phase1_surface_recursive_body_mutual_recursive_session_branch_spine_tree branch) ->
    (forall continuation_path continuation_input continuation_rest continuation,
      List.length continuation_input < parent_length ->
      Derives phase1_surface_rules continuation_path
        (ENonterminal "session_expression")
        continuation_input continuation_rest
        (phase1_surface_recursive_body_mutual_recursive_session_spine_tree continuation) ->
      exists adapter_fuel traversal_fuel converted,
        phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
          adapter_fuel traversal_fuel continuation = Some converted) ->
    exists adapter_fuel traversal_fuel refined,
      phase1_surface_normalize_recursive_body_transitive_session_branch_fuels
        adapter_fuel traversal_fuel branch = Some refined.
Proof.
  intros parent_length path input rest branch Hinput Hderive Hsolver.
  destruct
    (phase1_surface_recursive_body_mutual_branch_exposes_continuation
      path input rest branch Hderive)
    as [continuation_path [continuation_input [continuation_rest
      [Hcontinuation Hcontinuation_length]]]].
  assert (Hcontinuation_bound :
    List.length continuation_input < parent_length) by lia.
  destruct
    (Hsolver continuation_path continuation_input continuation_rest
      (match branch with
       | Phase1RecursiveBodyMutualRecursiveBranch _ _ _ _ continuation =>
           continuation
       end)
      Hcontinuation_bound Hcontinuation)
    as [adapter_fuel [traversal_fuel [converted Hconverted]]].
  exists adapter_fuel, traversal_fuel.
  unfold phase1_surface_normalize_recursive_body_transitive_session_branch_fuels,
    phase1_surface_normalize_recursive_body_transitive_session_branch_with.
  destruct branch as [label params boundary guard continuation].
  cbn in Hconverted |- *.
  rewrite Hconverted.
  eexists.
  reflexivity.
Qed.

Lemma phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels_total_from_repetition :
  forall parent_length path input rest branches,
    List.length input < parent_length ->
    DerivesRepetition phase1_surface_rules path
      phase1_surface_session_branch_suffix_expression_for_totality
      input rest
      (phase1_surface_recursive_body_mutual_recursive_session_branch_tail_spine_trees branches) ->
    (forall continuation_path continuation_input continuation_rest continuation,
      List.length continuation_input < parent_length ->
      Derives phase1_surface_rules continuation_path
        (ENonterminal "session_expression")
        continuation_input continuation_rest
        (phase1_surface_recursive_body_mutual_recursive_session_spine_tree continuation) ->
      exists adapter_fuel traversal_fuel converted,
        phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
          adapter_fuel traversal_fuel continuation = Some converted) ->
    exists adapter_fuel traversal_fuel refined,
      phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels
        adapter_fuel traversal_fuel branches = Some refined.
Proof.
  intros parent_length path input rest branches Hinput Hderive Hsolver.
  revert input rest Hinput Hderive.
  induction branches as [|branch branches IH];
    intros input rest Hinput Hderive.
  - cbn in Hderive.
    inversion Hderive; subst.
    exists 0, 0, Phase1RecursiveBodyTransitiveBranchTailNil.
    reflexivity.
  - cbn in Hderive.
    inversion Hderive as
      [|path0 body0 input0 middle0 rest0 tree0 trees0
         Hbody Hprogress Hrest]; subst.
    unfold phase1_surface_session_branch_suffix_expression_for_totality in Hbody.
    destruct
      (derives_sequence_expression_exposes_items
        phase1_surface_rules
        (descend path AtRepetitionBody)
        [ ELiteral "|"; ENonterminal "session_branch" ]
        input middle0
        (phase1_surface_session_branch_suffix_tree
          (phase1_surface_recursive_body_mutual_recursive_session_branch_spine_tree branch))
        Hbody)
      as [trees [Htree Hitems]].
    cbn [phase1_surface_session_branch_suffix_tree] in Htree.
    inversion Htree; subst trees.
    repeat match goal with
    | Hseq : DerivesSequence phase1_surface_rules _ _ (_ :: _) _ _ _ |- _ =>
        inversion Hseq; subst; clear Hseq
    end.
    match goal with
    | Hnil : DerivesSequence phase1_surface_rules _ _ [] _ _ _ |- _ =>
        inversion Hnil; subst; clear Hnil
    end.
    match goal with
    | Hpipe : Derives phase1_surface_rules _
        (ELiteral "|") ?pipe_input ?branch_input _,
      Hbranch : Derives phase1_surface_rules _
        (ENonterminal "session_branch") ?branch_input ?branch_rest
        (phase1_surface_recursive_body_mutual_recursive_session_branch_spine_tree branch) |- _ =>
        pose proof
          (phase1_surface_derivation_rest_length_le_input
            _ _ _ _ _ Hpipe) as Hpipe_length;
        assert (Hbranch_bound :
          List.length branch_input < parent_length) by lia;
        destruct
          (phase1_surface_normalize_recursive_body_transitive_session_branch_fuels_total_from_derivation
            parent_length _ branch_input branch_rest branch
            Hbranch_bound Hbranch Hsolver)
          as [branch_adapter [branch_traversal
            [refined_branch Hbranch_normalize]]]
    end.
    pose proof
      (phase1_surface_derivation_rest_length_le_input
        _ _ _ _ _ Hbody) as Hbody_length.
    assert (Hrest_bound : List.length middle0 < parent_length) by lia.
    destruct (IH middle0 rest0 Hrest_bound Hrest)
      as [rest_adapter [rest_traversal [refined_rest Hrest_normalize]]].
    let adapter_fuel := constr:(Nat.max branch_adapter rest_adapter) in
    let traversal_fuel := constr:(Nat.max branch_traversal rest_traversal) in
    pose proof
      (phase1_surface_normalize_recursive_body_transitive_session_branch_fuels_monotone
        branch_adapter adapter_fuel
        branch_traversal traversal_fuel
        branch refined_branch
        (Nat.le_max_l branch_adapter rest_adapter)
        (Nat.le_max_l branch_traversal rest_traversal)
        Hbranch_normalize) as Hbranch_lifted.
    pose proof
      (phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels_monotone
        rest_adapter adapter_fuel
        rest_traversal traversal_fuel
        branches refined_rest
        (Nat.le_max_r branch_adapter rest_adapter)
        (Nat.le_max_r branch_traversal rest_traversal)
        Hrest_normalize) as Hrest_lifted.
    exists adapter_fuel, traversal_fuel,
      (Phase1RecursiveBodyTransitiveBranchTailCons refined_branch refined_rest).
    unfold phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels.
    cbn.
    fold phase1_surface_normalize_recursive_body_transitive_session_branch_fuels.
    fold phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels.
    rewrite Hbranch_lifted, Hrest_lifted.
    reflexivity.
Qed.

Lemma phase1_surface_normalize_recursive_body_transitive_session_choice_fuels_total_from_derivation :
  forall direction path input rest choice,
    Derives phase1_surface_rules path
      (phase1_surface_session_choice_expression_for_totality direction)
      input rest
      (phase1_surface_recursive_body_mutual_recursive_session_choice_spine_tree choice) ->
    (forall continuation_path continuation_input continuation_rest continuation,
      List.length continuation_input < List.length input ->
      Derives phase1_surface_rules continuation_path
        (ENonterminal "session_expression")
        continuation_input continuation_rest
        (phase1_surface_recursive_body_mutual_recursive_session_spine_tree continuation) ->
      exists adapter_fuel traversal_fuel converted,
        phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
          adapter_fuel traversal_fuel continuation = Some converted) ->
    exists adapter_fuel traversal_fuel refined,
      phase1_surface_normalize_recursive_body_transitive_session_choice_fuels
        adapter_fuel traversal_fuel choice = Some refined.
Proof.
  intros direction path input rest
    [choice_direction first_branch rest_branches] Hderive Hsolver.
  unfold phase1_surface_session_choice_expression_for_totality in Hderive.
  destruct
    (derives_sequence_expression_exposes_items
      phase1_surface_rules path
      [ ELiteral (phase1_surface_session_choice_keyword direction);
        ELiteral "{";
        ENonterminal "session_branch";
        ERepetition phase1_surface_session_branch_suffix_expression_for_totality;
        ELiteral "}"
      ]
      input rest
      (phase1_surface_recursive_body_mutual_recursive_session_choice_spine_tree
        (Phase1RecursiveBodyMutualRecursiveChoice
          choice_direction first_branch rest_branches))
      Hderive)
    as [trees [Htree Hitems]].
  cbn in Htree.
  inversion Htree; subst trees.
  repeat match goal with
  | Hseq : DerivesSequence phase1_surface_rules _ _ (_ :: _) _ _ _ |- _ =>
      inversion Hseq; subst; clear Hseq
  end.
  match goal with
  | Hnil : DerivesSequence phase1_surface_rules _ _ [] _ _ _ |- _ =>
      inversion Hnil; subst; clear Hnil
  end.
  match goal with
  | Hkeyword : Derives phase1_surface_rules _
      (ELiteral (phase1_surface_session_choice_keyword direction))
      ?keyword_input ?keyword_rest ?keyword_tree |- _ =>
      destruct
        (literal_derivation_is_exact
          phase1_surface_rules _
          (phase1_surface_session_choice_keyword direction)
          keyword_input keyword_rest keyword_tree Hkeyword)
        as [keyword_tail
          [Hkeyword_input [Hkeyword_rest Hkeyword_tree_exact]]];
      assert (Hkeyword_length :
        List.length keyword_rest < List.length keyword_input);
      [ rewrite Hkeyword_input, Hkeyword_rest; cbn; lia
      | idtac ]
  end.
  match goal with
  | Hopen : Derives phase1_surface_rules _
      (ELiteral "{") ?open_input ?first_input _,
    Hfirst : Derives phase1_surface_rules _
      (ENonterminal "session_branch") ?first_input ?repetition_input
      (phase1_surface_recursive_body_mutual_recursive_session_branch_spine_tree first_branch),
    Hrest : Derives phase1_surface_rules _
      (ERepetition phase1_surface_session_branch_suffix_expression_for_totality)
      ?repetition_input ?after_repetition
      (PTRepetition
        (phase1_surface_recursive_body_mutual_recursive_session_branch_tail_spine_trees
          rest_branches)) |- _ =>
      pose proof
        (phase1_surface_derivation_rest_length_le_input
          _ _ _ _ _ Hopen) as Hopen_length;
      assert (Hfirst_bound :
        List.length first_input < List.length input) by lia;
      destruct
        (phase1_surface_normalize_recursive_body_transitive_session_branch_fuels_total_from_derivation
          (List.length input) _ first_input repetition_input first_branch
          Hfirst_bound Hfirst Hsolver)
        as [first_adapter [first_traversal
          [refined_first Hfirst_normalize]]];
      pose proof
        (phase1_surface_derivation_rest_length_le_input
          _ _ _ _ _ Hfirst) as Hfirst_length;
      assert (Hrest_bound :
        List.length repetition_input < List.length input) by lia;
      destruct
        (phase1_surface_repetition_derivation_exposes
          _ phase1_surface_session_branch_suffix_expression_for_totality
          _ _
          (PTRepetition
            (phase1_surface_recursive_body_mutual_recursive_session_branch_tail_spine_trees
              rest_branches))
          Hrest)
        as [rest_trees [Hrest_tree Hrest_derive]];
      cbn in Hrest_tree;
      inversion Hrest_tree; subst rest_trees;
      destruct
        (phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels_total_from_repetition
          (List.length input) _ repetition_input after_repetition
          rest_branches Hrest_bound Hrest_derive Hsolver)
        as [rest_adapter [rest_traversal
          [refined_rest Hrest_normalize]]];
      let adapter_fuel := constr:(Nat.max first_adapter rest_adapter) in
      let traversal_fuel := constr:(Nat.max first_traversal rest_traversal) in
      pose proof
        (phase1_surface_normalize_recursive_body_transitive_session_branch_fuels_monotone
          first_adapter adapter_fuel
          first_traversal traversal_fuel
          first_branch refined_first
          (Nat.le_max_l first_adapter rest_adapter)
          (Nat.le_max_l first_traversal rest_traversal)
          Hfirst_normalize) as Hfirst_lifted;
      pose proof
        (phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels_monotone
          rest_adapter adapter_fuel
          rest_traversal traversal_fuel
          rest_branches refined_rest
          (Nat.le_max_r first_adapter rest_adapter)
          (Nat.le_max_r first_traversal rest_traversal)
          Hrest_normalize) as Hrest_lifted;
      exists adapter_fuel, traversal_fuel,
        (Phase1RecursiveBodyTransitiveChoice
          choice_direction refined_first refined_rest);
      unfold phase1_surface_normalize_recursive_body_transitive_session_choice_fuels,
        phase1_surface_normalize_recursive_body_transitive_session_choice_with;
      cbn;
      fold phase1_surface_normalize_recursive_body_transitive_session_branch_fuels;
      fold phase1_surface_normalize_recursive_body_transitive_session_branch_tail_fuels;
      rewrite Hfirst_lifted, Hrest_lifted;
      reflexivity
  end.
Qed.

Lemma phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels_total_measure :
  forall measure path input rest session,
    List.length input < measure ->
    Derives phase1_surface_rules path
      (ENonterminal "session_expression") input rest
      (phase1_surface_recursive_body_mutual_recursive_session_spine_tree session) ->
    exists adapter_fuel traversal_fuel refined,
      phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
        adapter_fuel traversal_fuel session = Some refined.
Proof.
  induction measure as [|measure IH];
    intros path input rest session Hmeasure Hderive.
  - lia.
  - assert (Hsolver :
      forall continuation_path continuation_input continuation_rest continuation,
        List.length continuation_input < List.length input ->
        Derives phase1_surface_rules continuation_path
          (ENonterminal "session_expression")
          continuation_input continuation_rest
          (phase1_surface_recursive_body_mutual_recursive_session_spine_tree continuation) ->
        exists adapter_fuel traversal_fuel converted,
          phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
            adapter_fuel traversal_fuel continuation = Some converted).
    {
      intros continuation_path continuation_input continuation_rest
        continuation Hshorter Hcontinuation.
      assert (Hrecursive_measure :
        List.length continuation_input < measure) by lia.
      eapply IH.
      - exact Hrecursive_measure.
      - exact Hcontinuation.
    }
    destruct session as [nonreference | reference_tree].
    + destruct nonreference as
        [direction parameter boundary guard continuation
        |choice
        |choice
        |terminal
        |payload
        |payload].
      * let transfer := constr:(
          {| phase1_guard_refined_transfer_direction := direction;
             phase1_guard_refined_transfer_parameter := parameter;
             phase1_guard_refined_transfer_boundary := boundary;
             phase1_guard_refined_transfer_guard := guard;
             phase1_guard_refined_transfer_continuation_tree :=
               phase1_surface_recursive_body_mutual_recursive_session_spine_tree
                 continuation |}) in
        assert (Htransfer_tree :
          phase1_surface_guard_refined_boundary_typed_transfer_session_spine_tree
            (Phase1GuardRefinedBoundaryTypedTransferNonreferenceSession
              (Phase1GuardRefinedBoundaryTypedTransferSession transfer)) =
          phase1_surface_recursive_body_mutual_recursive_session_spine_tree
            (Phase1RecursiveBodyMutualRecursiveNonreferenceSession
              (Phase1RecursiveBodyMutualRecursiveTransferSession
                direction parameter boundary guard continuation)))
          by reflexivity;
        destruct
          (phase1_surface_guard_refined_transfer_continuation_derivation_shorter
            path input rest
            (phase1_surface_recursive_body_mutual_recursive_session_spine_tree
              (Phase1RecursiveBodyMutualRecursiveNonreferenceSession
                (Phase1RecursiveBodyMutualRecursiveTransferSession
                  direction parameter boundary guard continuation)))
            transfer Hderive Htransfer_tree)
          as [continuation_path [continuation_input [continuation_rest
            [Hcontinuation Hshorter]]]];
        destruct
          (Hsolver continuation_path continuation_input continuation_rest
            continuation Hshorter Hcontinuation)
          as [adapter_fuel [traversal_fuel [converted Hconverted]]];
        exists adapter_fuel, (S traversal_fuel),
          (Phase1RecursiveBodyTransitiveNonreferenceSession
            (Phase1RecursiveBodyTransitiveTransferSession
              direction parameter boundary guard converted));
        unfold phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels;
        cbn [phase1_surface_normalize_recursive_body_transitive_session_spine_fuel];
        change
          (phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
            adapter_fuel traversal_fuel continuation = Some converted)
          in Hconverted;
        rewrite Hconverted;
        reflexivity.
      * destruct
          (phase1_surface_selected_choice_derivation
            Phase1SessionSelect path input rest
            (phase1_surface_recursive_body_mutual_recursive_session_choice_spine_tree choice)
            Hderive)
          as [choice_path Hchoice];
        destruct
          (phase1_surface_normalize_recursive_body_transitive_session_choice_fuels_total_from_derivation
            Phase1SessionSelect choice_path input rest choice Hchoice Hsolver)
          as [adapter_fuel [traversal_fuel [refined_choice Hchoice_normalize]]];
        exists adapter_fuel, (S traversal_fuel),
          (Phase1RecursiveBodyTransitiveNonreferenceSession
            (Phase1RecursiveBodyTransitiveSelectSession refined_choice));
        unfold phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels;
        cbn [phase1_surface_normalize_recursive_body_transitive_session_spine_fuel];
        change
          (match
            phase1_surface_normalize_recursive_body_transitive_session_choice_fuels
              adapter_fuel traversal_fuel choice
          with
          | Some refined =>
              Some
                (Phase1RecursiveBodyTransitiveNonreferenceSession
                  (Phase1RecursiveBodyTransitiveSelectSession refined))
          | None => None
          end =
          Some
            (Phase1RecursiveBodyTransitiveNonreferenceSession
              (Phase1RecursiveBodyTransitiveSelectSession refined_choice)));
        rewrite Hchoice_normalize;
        reflexivity.
      * destruct
          (phase1_surface_selected_choice_derivation
            Phase1SessionOffer path input rest
            (phase1_surface_recursive_body_mutual_recursive_session_choice_spine_tree choice)
            Hderive)
          as [choice_path Hchoice];
        destruct
          (phase1_surface_normalize_recursive_body_transitive_session_choice_fuels_total_from_derivation
            Phase1SessionOffer choice_path input rest choice Hchoice Hsolver)
          as [adapter_fuel [traversal_fuel [refined_choice Hchoice_normalize]]];
        exists adapter_fuel, (S traversal_fuel),
          (Phase1RecursiveBodyTransitiveNonreferenceSession
            (Phase1RecursiveBodyTransitiveOfferSession refined_choice));
        unfold phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels;
        cbn [phase1_surface_normalize_recursive_body_transitive_session_spine_fuel];
        change
          (match
            phase1_surface_normalize_recursive_body_transitive_session_choice_fuels
              adapter_fuel traversal_fuel choice
          with
          | Some refined =>
              Some
                (Phase1RecursiveBodyTransitiveNonreferenceSession
                  (Phase1RecursiveBodyTransitiveOfferSession refined))
          | None => None
          end =
          Some
            (Phase1RecursiveBodyTransitiveNonreferenceSession
              (Phase1RecursiveBodyTransitiveOfferSession refined_choice)));
        rewrite Hchoice_normalize;
        reflexivity.
      * exists 0, 1,
          (Phase1RecursiveBodyTransitiveNonreferenceSession
            (Phase1RecursiveBodyTransitiveEndSession terminal)).
        reflexivity.
      * destruct payload as [name body].
        cbn in Hderive.
        destruct
          (phase1_surface_selected_recursive_derivation
            path input rest
            (phase1_surface_recursive_body_one_step_payload_spine_tree
              {| phase1_recursive_body_one_step_name := name;
                 phase1_recursive_body_one_step_body := body |})
            Hderive)
          as [recursive_path Hrecursive].
        destruct
          (phase1_surface_recursive_body_one_step_payload_exposes_body_shorter
            recursive_path input rest
            {| phase1_recursive_body_one_step_name := name;
               phase1_recursive_body_one_step_body := body |}
            Hrecursive)
          as [body_path [body_input [body_rest [Hbody Hbody_shorter]]]].
        destruct
          (phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel_total_from_derivation
            body_path body_input body_rest body Hbody)
          as [body_adapter [body_step [Hbody_adapter Hbody_tree]]].
        rewrite <- Hbody_tree in Hbody.
        destruct
          (Hsolver body_path body_input body_rest body_step Hbody_shorter Hbody)
          as [recursive_adapter [recursive_traversal
            [converted_body Hrecursive_body]]].
        let adapter_fuel := constr:(Nat.max body_adapter recursive_adapter) in
        pose proof
          (phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel_monotone
            body_adapter adapter_fuel body body_step
            (Nat.le_max_l body_adapter recursive_adapter) Hbody_adapter)
          as Hbody_adapter_lifted.
        pose proof
          (phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels_monotone
            recursive_adapter adapter_fuel
            recursive_traversal recursive_traversal
            body_step converted_body
            (Nat.le_max_r body_adapter recursive_adapter)
            (Nat.le_refl recursive_traversal)
            Hrecursive_body)
          as Hrecursive_lifted.
        exists adapter_fuel, (S recursive_traversal),
          (Phase1RecursiveBodyTransitiveNonreferenceSession
            (Phase1RecursiveBodyTransitiveRecursiveSession name converted_body));
        unfold phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels;
        cbn [phase1_surface_normalize_recursive_body_transitive_session_spine_fuel];
        unfold phase1_surface_normalize_recursive_body_mutual_adapter_uniform_fuel
          in Hbody_adapter_lifted;
        rewrite Hbody_adapter_lifted;
        change
          (phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels
            adapter_fuel recursive_traversal body_step = Some converted_body)
          in Hrecursive_lifted;
        rewrite Hrecursive_lifted;
        reflexivity.
      * exists 0, 1,
          (Phase1RecursiveBodyTransitiveNonreferenceSession
            (Phase1RecursiveBodyTransitiveContinueSession payload)).
        reflexivity.
    + exists 0, 1,
        (Phase1RecursiveBodyTransitiveStaticReferenceSession reference_tree).
      reflexivity.
Qed.

Theorem phase1_surface_normalize_recursive_body_transitive_session_spine_fuel_total_from_derivation :
  forall path input rest session,
    Derives phase1_surface_rules path
      (ENonterminal "session_expression") input rest
      (phase1_surface_recursive_body_mutual_recursive_session_spine_tree session) ->
    exists source_fuel closure_fuel transfer_fuel mutual_fuel traversal_fuel refined,
      phase1_surface_normalize_recursive_body_transitive_session_spine_fuel
        source_fuel closure_fuel transfer_fuel mutual_fuel traversal_fuel session =
        Some refined /\
      phase1_surface_recursive_body_transitive_session_spine_tree refined =
        phase1_surface_recursive_body_mutual_recursive_session_spine_tree session.
Proof.
  intros path input rest session Hderive.
  destruct
    (phase1_surface_normalize_recursive_body_transitive_session_spine_uniform_fuels_total_measure
      (S (List.length input)) path input rest session
      (Nat.lt_succ_diag_r (List.length input)) Hderive)
    as [adapter_fuel [traversal_fuel [refined Hnormalize]]].
  exists adapter_fuel, adapter_fuel, adapter_fuel, adapter_fuel,
    traversal_fuel, refined.
  split.
  - exact Hnormalize.
  - eapply
      phase1_surface_normalize_recursive_body_transitive_session_spine_fuel_round_trip.
    exact Hnormalize.
Qed.
