...
(* Converse for the select/offer branch-list shell introduced by #1033. *)

Definition phase1_surface_session_branch_suffix_expression_for_totality
  : EbnfExpression :=
  ESequence
    [ ELiteral "|";
...
Definition phase1_surface_session_choice_expression_for_totality
  (direction : Phase1SurfaceSessionChoiceDirection) : EbnfExpression :=
  ESequence
...
Lemma phase1_surface_normalize_session_branch_suffix_total_from_derivation :
...
Proof.
  ...
  unfold phase1_surface_session_branch_suffix_expression_for_totality in Hderive.
  destruct
    ...
    Hbranch : Derives phase1_surface_rules _
      ...
      pose proof
        (phase1_surface_validate_named_node_total_from_derivation
          "session_branch" _ _ _ branch_tree Hbranch) as Hbranch_validate;
      assert (Hnormalize :
        phase1_surface_normalize_session_branch_suffix tree = Some branch_tree);
...
Lemma phase1_surface_normalize_session_branch_suffixes_total_from_repetition :
...
Proof.
  ...
  - subst body.
    destruct
      (phase1_surface_normalize_session_branch_suffix_total_from_derivation
        (descend path AtRepetitionBody)
        input middle tree Hbody)
      as [branch_tree [Htree Htree_round_trip]].
    destruct (IHrest eq_refl) as [branches Hbranches].
    exists (branch_tree :: branches).
    cbn.
    rewrite Htree.
    rewrite Hbranches.
    reflexivity.
...
Theorem phase1_surface_normalize_session_choice_spine_total_from_derivation :
...
Proof.
  ...
  | Hkeyword : Derives phase1_surface_rules _
    ...
    Hclose : Derives phase1_surface_rules _
      ...
      destruct
        (phase1_surface_normalize_session_branch_suffixes_total_from_repetition
          _ _ _ _ _ Hrest_derive eq_refl)
        as [rest_branches Hrest_normalize];
      destruct
        (literal_derivation_is_exact
      ...
      let choice := constr:(
        {| phase1_session_choice_direction := direction;
           ...
           phase1_session_choice_rest_branch_trees := rest_branches |}) in
      assert (Hnormalize :
