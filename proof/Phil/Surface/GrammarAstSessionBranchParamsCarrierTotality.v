...
From Phil.Surface Require Import
  ...
  GrammarAstSessionBranchLabelCarrierTotality
...
(* Converse for the parameterized select/offer branch carrier from #1049. *)

Definition phase1_surface_session_branch_term_param_suffix_expression_for_totality
  : EbnfExpression :=
  ESequence
    [ ELiteral ",";
...
Definition phase1_surface_session_branch_term_param_list_expression_for_totality
  : EbnfExpression :=
  EOptional
    (ESequence
      [ ENonterminal "term_param";
        ERepetition
          phase1_surface_session_branch_term_param_suffix_expression_for_totality
      ]).

Lemma
  phase1_surface_normalize_session_branch_term_param_suffix_total_from_derivation :
...
Proof.
  ...
  unfold phase1_surface_session_branch_term_param_suffix_expression_for_totality
    in Hderive.
  destruct
  ...
  | Hcomma : Derives phase1_surface_rules _
    ...
    Hparameter : Derives phase1_surface_rules _
      (ENonterminal "term_param") _ _ ?parameter_tree |- _ =>
      destruct
      ...
      [ rewrite Htree, Hcomma_tree;
        unfold phase1_surface_normalize_session_branch_term_param_suffix,
...
Lemma
  phase1_surface_normalize_session_branch_term_param_suffixes_total_from_repetition :
  forall path body input rest trees,
    DerivesRepetition phase1_surface_rules path body input rest trees ->
    body = phase1_surface_session_branch_term_param_suffix_expression_for_totality ->
    exists parameters,
...
Lemma phase1_surface_normalize_session_branch_term_param_list_total_from_derivation :
  forall path input rest tree,
    Derives phase1_surface_rules path
      phase1_surface_session_branch_term_param_list_expression_for_totality
      input rest tree ->
    exists parameters,
...
Proof.
  intros path input rest tree Hderive.
  unfold phase1_surface_session_branch_term_param_list_expression_for_totality
    in Hderive.
  destruct
  ...
  - assert (Hnormalize :
      phase1_surface_normalize_session_branch_term_param_list tree = Some []).
    {
      rewrite Hnone.
      reflexivity.
    }
    ...
    split.
    + exact Hnormalize.
    + eapply phase1_surface_normalize_session_branch_term_param_list_round_trip.
      exact Hnormalize.
  - destruct
      (derives_sequence_expression_exposes_items
    ...
    | Hfirst : Derives phase1_surface_rules _
        (ENonterminal "term_param") _ _ ?first_tree,
      Hrest : Derives phase1_surface_rules _
        ...
        destruct
          (phase1_surface_repetition_derivation_exposes
            _ phase1_surface_session_branch_term_param_suffix_expression_for_totality
            _ _ rest_tree Hrest)
        ...
        let parameters := constr:(first :: rest_parameters) in
        assert (Hnormalize :
          phase1_surface_normalize_session_branch_term_param_list tree =
            Some parameters);
        [ rewrite Hsome, Htree, Hrest_tree;
          unfold phase1_surface_normalize_session_branch_term_param_list,
...
Lemma phase1_surface_normalize_session_branch_params_total_from_derivation :
  forall path input rest tree,
    Derives phase1_surface_rules path
...
Proof.
  intros path input rest tree Hderive.
  unfold phase1_surface_session_branch_params_expression_for_totality in Hderive.
  destruct
    (optional_derivation_exposes_presence
      phase1_surface_rules path
      (ESequence
    ...
    as [[_ Hnone] | [body_tree [Hsome Hbody]]].
  - assert (Hnormalize :
      phase1_surface_normalize_session_branch_params tree = Some None).
    {
      rewrite Hnone.
      reflexivity.
    }
    ...
    split.
    + exact Hnormalize.
    + eapply phase1_surface_normalize_session_branch_params_round_trip.
      exact Hnormalize.
  - change
      (Derives phase1_surface_rules (descend path AtOptionalBody)
        (ESequence
          [ ELiteral "(";
            phase1_surface_session_branch_term_param_list_expression_for_totality;
            ELiteral ")"
          ])
        input rest body_tree) in Hbody.
    destruct
      (derives_sequence_expression_exposes_items
        phase1_surface_rules
        (descend path AtOptionalBody)
    ...
    | Hopen : Derives phase1_surface_rules _
      ...
      Hparameters : Derives phase1_surface_rules _
        phase1_surface_session_branch_term_param_list_expression_for_totality
        _ _ ?parameters_tree,
      Hclose : Derives phase1_surface_rules _
        ...
        destruct
          (phase1_surface_normalize_session_branch_term_param_list_total_from_derivation
            _ _ _ parameters_tree Hparameters)
          as [values [Hparameters_normalize Hparameters_round_trip]];
        destruct
          (literal_derivation_is_exact
            phase1_surface_rules _ ")" _ _ close_tree Hclose)
          as [close_tail [_ [_ Hclose_tree]]];
        assert (Hnormalize :
          phase1_surface_normalize_session_branch_params tree =
            Some (Some values));
        [ rewrite Hsome, Htree, Hopen_tree, Hclose_tree;
          unfold phase1_surface_normalize_session_branch_params,
            phase1_surface_expect_optional,
            phase1_surface_expect_sequence,
...
Lemma phase1_surface_normalize_parameterized_session_branch_total_from_derivation :
  forall path input rest tree,
    Derives phase1_surface_rules path
      (ENonterminal "session_branch") input rest tree ->
    exists branch labeled parameterized,
...
Lemma
  phase1_surface_normalize_parameterized_session_branch_suffixes_total_from_repetition :
  forall path body input rest trees,
    DerivesRepetition phase1_surface_rules path body input rest trees ->
    body = phase1_surface_session_branch_suffix_expression_for_totality ->
    exists raw_branches structured_branches labeled_branches parameterized_branches,
    ...
    | Hpipe : Derives phase1_surface_rules _
      ...
      Hbranch : Derives phase1_surface_rules _
        ...
        pose proof
          ...
          as Hbranch_validate;
        destruct
          (phase1_surface_normalize_parameterized_session_branch_total_from_derivation
            _ _ _ branch_tree Hbranch)
          as [branch [labeled [parameterized
            [Hbranch_normalize [Hlabeled_normalize Hparameterized_normalize]]]]];
        assert (Hsuffix :
        ...
        | destruct (IHrest eq_refl)
            as [raw_branches [structured_branches [labeled_branches
              [parameterized_branches
                [Hraw_branches
                  [Hstructured_branches
                    [Hlabeled_branches Hparameterized_branches]]]]]]];
...
Theorem
  phase1_surface_normalize_parameterized_session_choice_tree_total_from_derivation :
  forall direction path input rest tree,
    ...
    exists choice,
      phase1_surface_normalize_parameterized_session_choice_tree direction tree =
        Some choice /\
      phase1_surface_parameterized_session_choice_spine_tree choice = tree.
Proof.
    ...
    Hclose : Derives phase1_surface_rules _
      ...
      destruct
        ...
        as [first_branch [first_labeled [first_parameterized
          [Hfirst_branch_normalize
            [Hfirst_labeled_normalize Hfirst_parameterized_normalize]]]]];
      destruct
        (phase1_surface_repetition_derivation_exposes
      ...
      destruct
        ...
        as [raw_rest [structured_rest [labeled_rest [parameterized_rest
          [Hraw_rest
            [Hstructured_rest [Hlabeled_rest Hparameterized_rest]]]]]]];
      destruct
        (literal_derivation_is_exact
          phase1_surface_rules _ "}" _ _ close_tree Hclose)
        as [close_tail [_ [_ Hclose_tree]]];
              ...
              | let choice := constr:(
                ...
                | assert (Hnormalize :
                  ...
                  | exists choice;
                    ...
                    [ exact Hnormalize
                    | eapply
                        phase1_surface_normalize_parameterized_session_choice_tree_round_trip;
                      exact Hnormalize ] ] ] ] ] ] ] ]
  end.
Qed.
