...
From Phil.Surface Require Import
  ...
  GrammarAstExpressionBaseChoiceTotality
  GrammarAstFallbackBaseChoiceTotality.

Import ListNotations.
Open Scope string_scope.

(* Converse for the expression-shell fallback-choice lift from #1317. *)

Lemma
  phase1_surface_normalize_optional_expression_fallback_base_choice_total_from_spine_derivation :
  forall fallback path input rest,
    ...
    exists refined,
      phase1_surface_normalize_optional_expression_fallback_base_choice fallback =
        Some refined.
Proof.
  intros [fallback_tree |] path input rest Hderive.
  - destruct
      (optional_derivation_exposes_presence
        ...
        input rest
        (phase1_surface_optional_expression_fallback_tree (Some fallback_tree))
        Hderive)
      as [[_ Hnone] | [body [Hsome Hbody]]].
    + cbn in Hnone.
      discriminate Hnone.
    + cbn in Hsome.
      inversion Hsome; subst body.
      destruct
        (derives_sequence_expression_exposes_items
          ...
          input rest
          (phase1_surface_expression_fallback_tail_tree fallback_tree)
          Hbody)
        as [trees [Hsequence_tree Hitems]].
      unfold phase1_surface_expression_fallback_tail_tree in Hsequence_tree.
      ...
      match goal with
      | Hfallback : Derives phase1_surface_rules _
          (ENonterminal "fallback") _ _ fallback_tree |- _ =>
          destruct
            (phase1_surface_normalize_fallback_base_choice_spine_total_from_derivation
              _ _ _ fallback_tree Hfallback)
            as [refined [Hrefined Hround_trip]];
          exists (Some refined);
          cbn;
          rewrite Hrefined;
          reflexivity
...
Proof.
  ...
  unfold phase1_surface_expression_base_choice_spine_tree in Hderive.
  cbn in Hderive.
  destruct
  ...
  | Hfallback : Derives phase1_surface_rules _
      ...
      _ _
      (phase1_surface_optional_expression_fallback_tree fallback) |- _ =>
      destruct
        (phase1_surface_normalize_optional_expression_fallback_base_choice_total_from_spine_derivation
          fallback _ _ _ Hfallback)
        as [refined_fallback Hrefined_fallback];
      exists
        {| phase1_expression_fallback_base_choice_spine_base := base;
           phase1_expression_fallback_base_choice_spine_fallback :=
             refined_fallback |};
      cbn;
      rewrite Hrefined_fallback;
      reflexivity
  end.
Qed.
