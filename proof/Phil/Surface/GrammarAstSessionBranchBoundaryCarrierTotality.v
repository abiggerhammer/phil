...
From Phil.Surface Require Import
  ...
  GrammarAstSessionBranchParamsCarrierTotality
...
(* Converse for the boundary-refined select/offer branch carrier from #1052. *)

Lemma phase1_surface_normalize_session_branch_boundary_total_from_derivation :
...
Proof.
  ...
  unfold phase1_surface_session_branch_boundary_expression_for_totality in Hderive.
  destruct
  ...
  - assert (Hnormalize :
      phase1_surface_normalize_session_branch_boundary tree = Some None).
    {
      rewrite Hnone.
      reflexivity.
    }
    ...
    split.
    + exact Hnormalize.
    + eapply phase1_surface_normalize_session_branch_boundary_round_trip.
      exact Hnormalize.
  - destruct
    ...
    match goal with
    | Husing : Derives phase1_surface_rules _
        (ELiteral "using") _ _ ?using_tree,
      Hreference : Derives phase1_surface_rules _
        (ENonterminal "static_reference") _ _ ?reference_tree |- _ =>
        destruct
          (literal_derivation_is_exact
            phase1_surface_rules _ "using" _ _ using_tree Husing)
          as [using_tail [_ [_ Husing_tree]]];
        destruct
          (phase1_surface_normalize_static_type_arguments_reference_layers_total_from_derivation
            _ _ _ reference_tree Hreference)
          as [base [middle [reference
            [Hbase [Hmiddle [Hrefined Hreference_round_trip]]]]]];
        ...
        [ unfold phase1_surface_normalize_static_type_arguments_reference_tree;
          ...
          exact Hrefined
        | assert (Hnormalize :
            phase1_surface_normalize_session_branch_boundary tree =
              Some (Some reference));
          [ rewrite Hsome, Htree, Husing_tree;
            unfold phase1_surface_normalize_session_branch_boundary,
...
Lemma phase1_surface_normalize_boundary_refined_session_branch_total_from_derivation :
  forall path input rest tree,
    Derives phase1_surface_rules path
      (ENonterminal "session_branch") input rest tree ->
    exists branch labeled parameterized refined,
...
Lemma
  phase1_surface_normalize_boundary_refined_session_branch_suffixes_total_from_repetition :
  forall path body input rest trees,
    ...
    exists raw_branches structured_branches labeled_branches
      parameterized_branches refined_branches,
      ...
      phase1_surface_normalize_boundary_refined_session_branch_spines
        parameterized_branches = Some refined_branches.
Proof.
    ...
    | Hpipe : Derives phase1_surface_rules _
      ...
      Hbranch : Derives phase1_surface_rules _
        ...
        pose proof
          ...
          as Hbranch_validate;
        destruct
          (phase1_surface_normalize_boundary_refined_session_branch_total_from_derivation
            _ _ _ branch_tree Hbranch)
          as [branch [labeled [parameterized [refined
            [Hbranch_normalize
              [Hlabeled_normalize
                [Hparameterized_normalize Hrefined_normalize]]]]]]];
        assert (Hsuffix :
        ...
        | destruct (IHrest eq_refl)
          ...
          | cbn;
            ...
            reflexivity
          | cbn;
            rewrite Hrefined_normalize;
            rewrite Hrefined_branches;
            reflexivity ] ]
    end.
Qed.

Theorem
  phase1_surface_normalize_boundary_refined_session_choice_tree_total_from_derivation :
  forall direction path input rest tree,
    ...
    exists choice,
      phase1_surface_normalize_boundary_refined_session_choice_tree direction tree =
        Some choice /\
      phase1_surface_boundary_refined_session_choice_spine_tree choice = tree.
Proof.
  ...
  | Hkeyword : Derives phase1_surface_rules _
    ...
    Hclose : Derives phase1_surface_rules _
      ...
      destruct
        ...
        as [first_branch [first_labeled [first_parameterized [first_refined
          [Hfirst_branch_normalize
            [Hfirst_labeled_normalize
              [Hfirst_parameterized_normalize Hfirst_refined_normalize]]]]]]];
      destruct
      ...
      destruct
        ...
        as [raw_rest [structured_rest [labeled_rest [parameterized_rest
          [refined_rest
            [Hraw_rest
              [Hstructured_rest
                [Hlabeled_rest [Hparameterized_rest Hrefined_rest]]]]]]]]];
      destruct
        (literal_derivation_is_exact
                  ...
                  | let choice := constr:(
                      {| phase1_boundary_refined_session_choice_direction :=
                         ...
                         phase1_boundary_refined_session_choice_rest_branches :=
                           refined_rest |}) in
                    assert (Hrefined_choice :
                      phase1_surface_normalize_boundary_refined_session_choice_spine
                        parameterized_choice = Some choice);
                    [ unfold
                        phase1_surface_normalize_boundary_refined_session_choice_spine;
                      cbn;
                      rewrite Hfirst_refined_normalize;
                    ...
                    | assert (Hnormalize :
                      ...
                      [ unfold
                        ...
                        rewrite Hrefined_choice;
                        reflexivity
                      | exists choice;
                        split;
                        [ exact Hnormalize
                        | eapply
                            phase1_surface_normalize_boundary_refined_session_choice_tree_round_trip;
                          exact Hnormalize ] ] ] ] ] ] ] ] ] ]
  end.
Qed.
