...
import Phil.Assurance.RocqSystemsFoundations
...
main = do
  args <- getArgs
  case args of
    [checkedRoot, outputRoot] -> mapM_ (certifyOne checkedRoot outputRoot) systemsFoundationCertificationSpecs
    _ -> failWith "usage: phil-certify-systems-foundations CHECKED_ROOT OUTPUT_ROOT"
...
certifyOne checkedRoot outputRoot spec = do
  ...
  case certifyRocqProof spec sourceBytes objectBytes of
    Left err -> failWith ("Systems foundations back-certification failed for " <> Text.unpack obligation <> ": " <> show err)
    Right bundle -> do
