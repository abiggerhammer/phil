...
import Phil.Assurance.RocqSurfaceFoundations
...
main = do
  args <- getArgs
  case args of
    [checkedRoot, outputRoot] -> mapM_ (certifyOne checkedRoot outputRoot) surfaceFoundationCertificationSpecs
    _ -> failWith "usage: phil-certify-surface-foundations CHECKED_ROOT OUTPUT_ROOT"
...
certifyOne checkedRoot outputRoot spec = do
  ...
  case certifyRocqProof spec sourceBytes objectBytes of
    Left err -> failWith ("Surface foundations back-certification failed for " <> Text.unpack obligation <> ": " <> show err)
    Right bundle -> do
