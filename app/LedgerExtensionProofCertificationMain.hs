...
import Phil.Assurance.RocqLedgerExtension
...
main = do
  ...
  case args of
    [sourcePath, objectPath, outputPath] -> do
      ...
      case packageTrustedRocqProof ledgerExtensionCertificationSpec sourceBytes objectBytes of
        Left err -> failWith ("ledger-extension proof certification failed: " <> show err)
        Right bundle -> do
          ...
          putStrLn "certified PHIL-ASSURE-LEDGER-EXT-001"
          putStrLn
    ...
    _ -> failWith
      "usage: phil-certify-ledger-extension SOURCE.v SOURCE.vo OUTPUT.cert"
