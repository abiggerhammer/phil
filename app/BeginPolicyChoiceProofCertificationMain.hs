...
main :: IO ()
main = do
  let root = "rocq-begin-policy-choice" </> "proof" </> "Phil"
      output = "begin-policy-choice-certificates"
  ...
  final <- case phase0BeginPolicyChoiceProofCertification
      systemsBeginPolicyProof llvmBeginPolicyProof versionPredecessor of
    Left err -> failWith ("proof-bound PHIL-LLVM-CERT-011 failed: " <> show err)
    Right bundle -> pure bundle

  mapM_ (uncurry (writeProofCertificate output))
    [ ("PHIL-SYS-BEGIN-POLICY-001.rocq.cert", systemsBeginPolicyProof)
    , ("PHIL-LLVM-BEGIN-POLICY-001.rocq.cert", llvmBeginPolicyProof)
    ]

  let predecessorText = renderVersionSessionChoiceProofBoundCertification versionPredecessor
      predecessorArtifact = versionProofBoundArtifact versionPredecessor
      finalText = renderBeginPolicyChoiceProofCertification final
      finalArtifact = beginPolicyProofArtifact final
  ByteString.writeFile (output </> "PHIL-LLVM-CERT-010.proof-bound.cert")
    (TextEncoding.encodeUtf8 predecessorText)
  ByteString.writeFile (output </> "PHIL-LLVM-CERT-011.proof-bound.cert")
    (TextEncoding.encodeUtf8 finalText)

  putStrLn "reproduced proof-bound PHIL-LLVM-CERT-010 predecessor authority"
  putStrLn ("predecessor certificate sha256: " <>
    Text.unpack (unDigest (artifactDigest predecessorArtifact)))
  putStrLn "certified PHIL-LLVM-CERT-011 with proof-bound BeginPolicy authority"
