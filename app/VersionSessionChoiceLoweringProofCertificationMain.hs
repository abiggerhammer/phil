...
main = do
  ...
  let predecessorText = renderPayloadCancelChoiceProofCertification predecessor
      predecessorArtifact = payloadCancelChoiceProofCertificationArtifact predecessor
      finalText = renderVersionSessionChoiceProofBoundCertification final
      finalArtifact = versionProofBoundArtifact final
  ByteString.writeFile (output </> "PHIL-LLVM-CERT-009.proof-bound.cert")
  ...
  putStrLn "reproduced proof-bound PHIL-LLVM-CERT-009 predecessor authority"
  putStrLn ("predecessor certificate sha256: " <>
    Text.unpack (unDigest (artifactDigest predecessorArtifact)))
  putStrLn "certified PHIL-LLVM-CERT-010 with proof-bound version-choice authority"
