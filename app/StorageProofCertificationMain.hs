...
certifyRejectedResponse args = case args of
  [ systemsRejectedSource, systemsRejectedObject
    ...
    , predecessorCertificatePath, finalCertificatePath
      ...
      case phase0RejectedResponseProofCertification
          systemsRejectedProof llvmRejectedProof
          systemsAcceptedProof llvmAcceptedProof
          systemsStorageProof llvmStorageProof
          systemsDigestProof llvmDigestProof systemsRecordProof exactProof abiProof symbolProof of
        Left err -> failWith ("rejected-response proof certification failed: " <> show err)
        Right bundle -> do
          let predecessor = rejectedResponseProofCertificationPredecessor bundle
              predecessorText = renderAcceptedResponseProofCertification predecessor
              finalText = renderRejectedResponseProofCertification bundle
              finalArtifact = rejectedResponseProofCertificationArtifact bundle
          ByteString.writeFile predecessorCertificatePath
            (TextEncoding.encodeUtf8 predecessorText)
          ByteString.writeFile finalCertificatePath
            (TextEncoding.encodeUtf8 finalText)
          putStrLn "reproduced proof-bound PHIL-LLVM-CERT-006 predecessor authority"
          putStrLn
            ("predecessor certificate sha256: " <>
              Text.unpack (unDigest (artifactDigest
                (acceptedResponseProofCertificationArtifact predecessor))))
          putStrLn "certified PHIL-LLVM-CERT-007 with proof-bound rejected-response authority"
          putStrLn
            ("certificate artifact: " <>
              Text.unpack (unArtifactRef (artifactReference finalArtifact)))
          putStrLn
