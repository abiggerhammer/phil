...
import Phil.Assurance.RocqLLVMFoundations
...
main = do
  args <- getArgs
  case args of
    [checkedRoot, outputRoot] -> mapM_ (certifyOne checkedRoot outputRoot) llvmFoundationCertificationSpecs
    _ -> failWith "usage: phil-certify-llvm-foundations CHECKED_ROOT OUTPUT_ROOT"
...
certifyOne checkedRoot outputRoot spec = do
  ...
  case certifyRocqProof spec sourceBytes objectBytes of
    Left err -> failWith ("LLVM foundations back-certification failed for " <> Text.unpack obligation <> ": " <> show err)
    Right bundle -> do
