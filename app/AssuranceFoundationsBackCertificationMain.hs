...
import Phil.Assurance.RocqAssuranceFoundations
...
main = do
  ...
  case args of
    [checkedRoot, outputRoot] ->
      mapM_ (certifyOne checkedRoot outputRoot) assuranceFoundationCertificationSpecs
    _ -> failWith "usage: phil-certify-assurance-foundations CHECKED_ROOT OUTPUT_ROOT"
...
certifyOne checkedRoot outputRoot spec = do
  ...
  case packageTrustedRocqProof spec sourceBytes objectBytes of
    Left err -> failWith ("Assurance foundations back-certification failed for " <> Text.unpack obligation <> ": " <> show err)
    Right bundle -> do
