...
import Phil.Assurance.RocqRecognitionGates
...
main = do
  args <- getArgs
  case args of
    [checkedRoot, outputRoot] -> mapM_ (certifyOne checkedRoot outputRoot) recognitionGateCertificationSpecs
    _ -> failWith "usage: phil-certify-recognition-gates CHECKED_ROOT OUTPUT_ROOT"
...
certifyOne checkedRoot outputRoot spec = do
  ...
  case certifyRocqProof spec sourceBytes objectBytes of
    Left err -> failWith ("Recognition gate back-certification failed for " <> Text.unpack obligation <> ": " <> show err)
    Right bundle -> do
