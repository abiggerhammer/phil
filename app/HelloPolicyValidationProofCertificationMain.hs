...
main :: IO ()
main = do
  let root = "rocq-hello-policy-validation" </> "proof" </> "Phil"
      output = "hello-policy-validation-certificates"
  ...
  final <- case phase0HelloPolicyValidationProofCertification
      systemsHelloPolicyProof llvmHelloPolicyProof beginPredecessor of
    Left err -> failWith ("proof-bound PHIL-LLVM-CERT-012 failed: " <> show err)
    Right bundle -> pure bundle

  mapM_ (uncurry (writeProofCertificate output))
    [ ("PHIL-SYS-HELLO-POLICY-001.rocq.cert", systemsHelloPolicyProof)
    , ("PHIL-LLVM-HELLO-POLICY-001.rocq.cert", llvmHelloPolicyProof)
    ]

  let predecessorText = renderBeginPolicyChoiceProofCertification beginPredecessor
      predecessorArtifact = beginPolicyProofArtifact beginPredecessor
      finalText = renderHelloPolicyValidationProofCertification final
      finalArtifact = helloPolicyProofArtifact final
  ...
  putStrLn "reproduced proof-bound PHIL-LLVM-CERT-011 predecessor authority"
  putStrLn ("predecessor certificate sha256: " <>
    Text.unpack (unDigest (artifactDigest predecessorArtifact)))
  putStrLn "certified PHIL-LLVM-CERT-012 with proof-bound HelloPolicy authority"
