...
import Phil.Assurance (digestText)
import Phil.LLVM
...
testSourcePairBoundaryBinding :: IO Bool
testSourcePairBoundaryBinding = do
  let clientA = "x\nfile:server.phil\ny"
      serverA = "z"
      clientB = "x"
      serverB = "y\nfile:server.phil\nz"
      ...
      oldCollision = legacyDigest clientA serverA == legacyDigest clientB serverB
      newSeparation =
        phase0UploadSourceDigest clientA serverA /= phase0UploadSourceDigest clientB serverB
  if oldCollision && newSeparation
    then passCase "source-pair digest binds client/server file boundaries unambiguously"
    else failCase "source-pair boundary binding regression"
