...
module Phil.Phase0UploadProjection
...
phase0UploadSourceDigest clientSource serverSource =
  let clientDigest = digestText clientSource
      serverDigest = digestText serverSource
  in digestText $ Text.intercalate "\n"
      [ "phil-source-pair/phase0-upload/v1"
      , "client.phil=" <> unDigest clientDigest
      , "server.phil=" <> unDigest serverDigest
      ]
...
expressionSemanticTrace expression = case expression of
  ...
  ReceiveExactExpression lengthExpr endpoint evidence ->
    "receive_exact"
      : locatedExpressionTrace lengthExpr
