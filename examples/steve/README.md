...
The intended external handoff point is a coherent Steve package: design notes and obligations agree with the checker-facing witness, the witness and negative corpus parse in CI, known semantic checker gaps are explicitly classified, and the material is ready to present rather than merely ready to discuss.

## Intended architectural boundary

Steve depends on two authorities:

- `DigestProvider[SHA256]`: compute a content ID over an immutable byte view and check whether a byte object matches a supplied content ID.
...
Persistent evidence is indexed by a stable byte-object identity, not by the temporary shared loan through which the bytes were inspected. A digest provider may observe a `BorrowedBytes` view, but evidence surviving the borrow scope establishes `DigestMatches(contentId, byteObjectIdentity)`. This is the Steve specialization of Phil's existing stable-identity rule for evidence about restricted resources.
...
Branch CI parses every `.phil` file below `examples/steve/`, including the intentionally rejected semantic witnesses:

```text
find examples/steve -type f -name '*.phil' | sort
```

Each file is passed to:

```text
cabal run phil-core -- parse <file>
```

This is intentionally only a syntax gate. It must not be described as semantic acceptance of Steve or semantic rejection of the negative fixtures.

The positive sketch also makes exact byte comparison evidence-bearing: the `equal` arm carries evidence of bytewise equality between stable byte-object identities, while the `different` arm carries inequality evidence. On an `already_exists` path Steve compares first and computes the existing object's digest only if the byte strings differ.
...
3. **Generic decision/result types are not yet checker-defined.** The upload checker knows a fixed set of primitive decision shapes. Steve needs ordinary result variants such as `installed`, `already_exists`, `found`, `storage_failure`, `equal`, and `different`.
...
5. **Opaque claim/validator bindings are still architecture-environment data.** Steve needs `DigestMatches(ContentId, ByteObjectId)` tied specifically to `DigestProvider`, without granting arbitrary code authority to manufacture that evidence.
6. **Stable evidence identity must survive a borrow without the borrow escaping.** The checker needs to distinguish evidence indexed by the stable owner/snapshot identity from evidence illegally indexed by the loan token itself.

## Classification against accepted Phil decisions

The current pass does **not** reveal a need for Phil ADR-012.
...
- Stable identities in propositions are already part of ADR-001/ADR-002, and ADR-006's canonical opaque-digest example uses a stable payload identity while the validator observes a shared loan. Steve applies the same rule to byte objects.
...
- exact byte equality/inequality is explicit evidence on the relevant branch;
...
- successful `get` transfers owned bytes only after digest verification;
- persistent digest evidence never depends on an escaped borrow token;
