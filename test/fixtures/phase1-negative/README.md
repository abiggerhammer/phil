...
The validation/provenance tranche uses the same machinery for raw recognition input, `Begin` records, validation evidence, explicit receive/select requirements, and an owned payload witness. These are semantic boundary values rather than fixture-name aliases.
