...
The corpus intentionally covers the Grammar-v1/type-system reconciliation forms independently and in composition: refinement types, explicit transport, native membership/disjointness, term-level `offer`, branch-sensitive callable outcome residues, and exact replacement-callee transitions. It also covers declaration-level structural-mode syntax from the later reconciliation: record/sum modes, all three capability mode literals, ordinary bindings without local mode qualifiers, and representative malformed mode spellings.
