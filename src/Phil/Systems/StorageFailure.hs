...
data StorageFailureBundle = StorageFailureBundle
  ...
  , storageFailurePredecessor :: RecognitionFailureBundle
  , storageFailureWitness :: StorageFailureWitness
  }
  deriving (Eq, Show)

data StorageFailureError
  = StorageFailurePredecessorError RecognitionFailureError
  ...
  | StorageFailureRecognitionRegression RecognitionFailureError
...
phase0StorageFailureBundle = do
  predecessor <- mapLeft StorageFailurePredecessorError phase0RecognitionFailureBundle
  let baseArtifact = recognitionFailureArtifact predecessor
      baseContext = recognitionFailureContext predecessor
...
verifyStorageFailureBundle bundle = do
  mapLeft StorageFailurePredecessorError $
    verifyRecognitionFailureBundle (storageFailurePredecessor bundle)
  ...
  mapLeft StorageFailureRecognitionRegression $
  ...
  where
    provisional = LoweringDecision
      ...
      , loweringInvariantsPreserved =
          ...
          , "recognition failure details remain present and grammar-specific"
          ]
      ...
      , loweringTargetPreconditions =
          ...
          , "recognition failure detail predecessor verifies before storage failure materialization"
          ]
      , loweringAssumptions = []
      ...
      , loweringInspectionPlan =
          ...
          , "verify Hello and Begin recognition failure reasons remain exact and one-use"
          ]
      }
